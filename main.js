...
buttonElement.addEventListener('click', valor => {
    update(inputElement.value) //Executa a função Update
})

function update(valor) {
    let { dozeCasas, cavaleiros } = window; //Recebem os valores do arquivo Data.js
    if (valor >= 1 && valor <= 12) { //Verificação dos valores de cada Casa.
        ...
        alert('Digite um número de 1 e 12'); //Retorna um Alerta caso não seja digitado nenhum dos valores entre 1 e 12
    }
}
